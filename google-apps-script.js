...
function doPost(e) {
  try {
    ...
    if (action === 'addOriginalYearColumn') {
      const sheet = getSheet(COLLECTION_TAB);
      const lastCol = sheet.getLastColumn();
      const headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
      Logger.log('Current headers: ' + JSON.stringify(headers));
      Logger.log('Last column: ' + lastCol);
      const origIdx = headers.indexOf('originalYear');
      if (origIdx !== -1) {
        return respond({ ok: true, message: 'originalYear already exists at col ' + (origIdx + 1), headers });
      }
      // Add it at the next column
      const newCol = lastCol + 1;
      sheet.getRange(1, newCol).setValue('originalYear');
      // Verify
      const newHeaders = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      Logger.log('New headers: ' + JSON.stringify(newHeaders));
      return respond({ ok: true, message: 'Added originalYear at col ' + newCol, headers: newHeaders });
    }
