...
Which question index (just the number) would best narrow down the right record recommendation given their answers so far? Reply with only the number.`;
      ...
      try {
        const r = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({ model: 'claude-sonnet-4-5-20251022', max_tokens: 10, messages: [{ role: 'user', content: pickPrompt }] })
        });
        if (r.ok) {
  ...
  {"rank":3,"artist":"Artist","title":"Title","year":"1979","tagline":"short punchy tagline here"}
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
        body: JSON.stringify({ model: 'claude-sonnet-4-5-20251022', max_tokens: 400, messages: [{ role: 'user', content: prompt }] })
      });
