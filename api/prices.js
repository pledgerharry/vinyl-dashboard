...
async function getEbayPrice(artist, title, token) {
  if (!token) return null;
  try {
    const q = encodeURIComponent(`${artist} ${title} vinyl`);
    ...
    if (!r.ok) return null;
    const d = await r.json();
    const a0 = artist.toLowerCase().split(' ')[0];
    const t0 = title.toLowerCase().split(' ')[0];
    const prices = (d.itemSummaries || [])
      .filter(i => { const t = (i.title||'').toLowerCase(); return t.includes(a0)&&t.includes(t0)&&!t.includes('cd')&&!t.includes('box set'); })
      .map(i => parseFloat(i.price?.value||0))
      .filter(p => p > 0 && p < 300)
      .sort((a,b) => a-b);
    if (!prices.length) return null;
    // Remove outliers using IQR before taking median
