...
async function getMarketPrice(releaseId, headers) {
  // Strategy 1: marketplace stats — median of real completed GBP sales
  try {
    const r = await fetch(
    ...
    if (r.ok) {
      ...
      if (!d.blocked_from_sale && d.median?.value >= 3) return Math.round(d.median.value);
    }
  } catch {}

  // Strategy 2: price suggestions at VG+
  try {
    const r = await fetch(
...
function priceFromCommunity(r) {
  ...
  const ageMult = year < 1970 ? 1.3 : year < 1980 ? 1.1 : 1.0;
