...
export default async function handler(req, res) {
  ...
  if (mode === 'price_check') {
    ...
    const prompt = `You are advising a vinyl collector on whether to buy a record. Be a knowledgeable friend — direct, warm, specific. 2-3 sentences max.

Record: ${record.artist} — ${record.title} (${record.year}, ${record.genre})
${priceContext}

Their collection taste (sample): ${GENRE_CONTEXT}
Collection includes: ${collSummary}

Give a short verdict: is it worth buying at this price? Factor in the price relative to market value, how well it fits their taste, and any relevant context about the record itself (is it a classic? a sleeper? overrated?). Don't repeat the price stats — just give your honest take.

Respond ONLY with JSON: {"verdict": "Your 2-3 sentence verdict here"}`;
