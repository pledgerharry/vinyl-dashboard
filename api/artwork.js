  ...
  try {
    ...
    if (releaseId) {
      ...
      if (r.ok) {
        ...
        // Return original year if different from pressing year
        const originalYear = d.master_id && d.released
          ? String(d.released).slice(0,4)
          : null;
        const masterYear = d.master?.year ? String(d.master.year) : null;
        if (url) return res.status(200).json({ url, originalYear: masterYear || originalYear });
      }
    }
